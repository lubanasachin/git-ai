...
/// Uses the same diff engine as the single-commit stats path so that
/// renamed-and-rewritten files are represented as renames with edit hunks
/// instead of a full delete/add pair (see `get_git_diff_stats` in `stats.rs`).
fn get_git_diff_stats_for_range(
    ...
    use crate::authorship::stats::diff_line_counts_from_hunks;
    use crate::commands::diff::get_diff_with_line_numbers;

    let hunks = get_diff_with_line_numbers(repo, start_sha, end_sha)?;
